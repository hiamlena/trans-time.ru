...
def _feature_from_manual_only(manual: FrameManual):
    ...
    if manual.lon_override is None or manual.lat_override is None:
        return None  # без координат смысла нет
    ...
    try:
        ...
        for raw in raws:
            ...
            if m and m.is_deleted_by_admin:
                continue
            feat = _merge_raw_and_manual(raw, m)
            features.append(feat)

        # manual-only (без raw)
        for m in manual_only_list:
            if m.frame_id in [r.frame_id for r in raws]:
                continue
...
def suggest_for_existing_frame(frame_id: str):
    ...
    Тело JSON:
    {
      "comment_driver": "...",  -- обязательно
      "suggested_lon":  ...,
      "suggested_lat":  ...,
      "suggested_weight_limit_tons": ...,
      "suggested_axle_load_tons": ...,
      "suggested_direction": "...",
      "suggested_frame_state": "...",
      "contact_phone": "...",
      "contact_name": "..."
    }
    """
